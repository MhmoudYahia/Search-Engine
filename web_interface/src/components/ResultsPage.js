...
const ResultsPage = (props) => {
  ...
  const [toSearch, setToSearch] = React.useState('http://goatserver-production.up.railway.app/api?text=' + target);
  const [myResults, setMyResults] = React.useState(null);

  const { data, isPending, error,time,setIsPending } = useFetch(toSearch);
  React.useEffect(() => {
    setMyResults(null)
    setToSearch('http://goatserver-production.up.railway.app/api?text=' + target);
  }, [target]);
  React.useEffect((() => {
      setResults(data);
